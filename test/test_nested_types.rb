...
class TestNestedTypes < Test::Unit::TestCase
  ...
  should "parser should works when nested attribute contains parenthesis" do
    bar = Bar2.create!(nested: {nested: {comp: [1, 'dc)))a'], color: 'blue'}, color: 'red'})
    Bar2.all.to_a.last.nested
    assert true
  end

  should "parser should works when nested attribute contains double quote" do
  ...
  should "parser should works when nested attribute contains backslash" do
